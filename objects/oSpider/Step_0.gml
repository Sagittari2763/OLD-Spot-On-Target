...
mask_index = sSpiderIdle;
player_check(); if allow { //If player is not out of range
	if !death { //If not dead
		
	if place_meeting(x, y, oAttack) and dmgTimer <= 0 {hp--; //Take damage
	...
	if instance_exists(oPlayer) {
		player_check(225); if allow and oPlayer.dmgLagTimer <= 0 {
		acidTimer--; //Count down time
		if acidTimer = 40 {image_index = 0; sprite_index = sSpiderAtk;} //Change sprite
		if acidTimer = 15 {instance_create_depth(x+(2*moveDir), y+acidy, depth+1, oAcid);} //Create acid web
		if acidTimer <= 0 {acidTimer = acidFrames; sprite_index = sSpiderIdle;}} //Reset timer and sprite
		else {sprite_index = sSpiderIdle; acidTimer = acidFrames;}}
		
	image_xscale = -moveDir; //Change sprite direction
	image_speed = 1; //Play sprite
	
	} else { //If dead
		if deathLog = false {deathDir = oPlayer.image_xscale; deathLog = true;}
